...
interface ApiService {
    ...
    @GET("car") suspend fun getCar(): List<Cars>

    @GET("car/{id}") suspend fun getCar(@Path("id") id: String): Car

    @DELETE("car/{id}") suspend fun deleteCar(@Path("id") id: String)

    @POST("car") suspend fun addCar(@Body item: CarValue): Car

    @PATCH("car/id") suspend fun editCar(@Path("id") id: String, @Body item: CarValue): Car
}